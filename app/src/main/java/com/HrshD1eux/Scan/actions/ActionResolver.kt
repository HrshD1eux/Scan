...
class ActionResolver(private val context: Context, private val onShowQr: () -> Unit) {
    ...
    fun resolve(content: ParsedContent): List<Action> {
            ...
            is ParsedContent.Upi -> {
                actions.add(
                    Action("Pay with UPI", Icons.Default.Payment, isPrimary = true) {
                        content.rawUri?.let { launchIntent(Intent(Intent.ACTION_VIEW, it)) }
                    }
                )
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            try {
                                ...
                                if (content.password != null) {
                                    if (content.securityType.contains("WPA3", ignoreCase = true)) {
                                        suggestionBuilder.setWpa3Passphrase(content.password)
                                    } else if (content.securityType.contains("WPA", ignoreCase = true)) {
        ...
        actions.add(
            Action("Show QR Code", Icons.Default.QrCode) {
                onShowQr()
            }
        )
        return actions
